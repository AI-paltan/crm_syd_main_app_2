...
class DataDump:
    ...
    def get_month(self,):
        ...
        self.filename = os.path.splitext(file_query.filename)[0]
        ...
        for idx,row in bs_crm_nlp_df_sorted.iterrows():
            ...
            if row["cdm_keyword_start_row_map"] is not None:
                if row["cdm_keyword_start_row_map"].isdigit():
                    ...
                    main_page_cropped_df = repsonse_dict.get('main_page_cropped_df')
    ...
    def dump_ccf_data(self):
        year_excel_col_map_dict = self.get_years_excel_colmap(total_col=8)
        # print(year_excel_col_map_dict)
        ccf_worksheet = self.workbook.get_sheet_by_name("CF")
        for page_number,ccf_dict in self.ccf_response_bucket.items(): 
