...
def _tile_by_stretching_rows_rightwise_column_downwise(
    dp: Image, items: List[ImageAnnotation], table: ImageAnnotation, item_name: str
) -> None:
    if table.image is None:
        raise ValueError("table.image cannot be None")
    ...
    for idx, item in enumerate(items):
        with MappingContextManager(dp_name=dp.file_name):
            ...
            if idx != len(items) - 1:
                next_item_embedding_box = items[idx + 1].image.get_embedding(dp.image_id)  # type: ignore
                tmp_next_item_xy = (
                    (next_item_embedding_box.uly) 
                    if item_name == LayoutType.row
                    else (next_item_embedding_box.ulx) 
                )
            else:
                tmp_next_item_xy = (
                    table_embedding_box.lry - 1.0 if item_name == LayoutType.row else table_embedding_box.lrx - 1.0
            ...
            if idx != len(items) - 1:
                new_embedding_box = BoundingBox(
                    ulx=item_embedding_box.ulx,
                    uly=item_embedding_box.uly,
                    lrx=item_embedding_box.lrx if item_name == LayoutType.row else tmp_next_item_xy,
                    #lry=tmp_next_item_xy if item_name == LayoutType.row else item_embedding_box.lry,
                    lry= item_embedding_box.lry,
                    absolute_coords=True,
    ...
    #             ulx=item_embedding_box.ulx,
    #             uly=item_embedding_box.uly,
    #             lrx=item_embedding_box.lrx if item_name == LayoutType.row else tmp_item_xy,
    #             lry=tmp_item_xy if item_name == LayoutType.row else item_embedding_box.uly,
    #             absolute_coords=True,
    #         )
    ...
    #         tmp_item_xy = item_embedding_box.uly if item_name == LayoutType.row else item_embedding_box.ulx
    #         item.image.set_embedding(dp.image_id, new_embedding_box)
...
def tile_tables_with_items_per_table(
    ...
    :param stretch_rule: Tiling can be achieved by two different stretching rules for rows and columns.
                        ...
                        - 'right': The lower horizontal edge of a row will be shifted up to the upper horizontal edge
                                   of the lower neighboring row. Similarly, the right sided vertical edge of a column
                                   will be shifted towards the left sided vertical edge of the right sided neighboring
                                   column.
    :return: Image
    """
    ...
    elif stretch_rule=="right":
        _tile_by_stretching_rows_rightwise_column_downwise(dp, items, table, item_name)
    else:
...
class TableSegmentationService(PipelineComponent):
    ...
    def __init__(
        ...
        remove_iou_threshold_rows: float,
        remove_iou_threshold_cols: float,
        stretch_rule: Literal["left", "equal", "right"] = "right",
    ):
        """
        ...
        assert stretch_rule in ("left", "equal","right"), "stretch rule must be either 'left' or 'equal'"
