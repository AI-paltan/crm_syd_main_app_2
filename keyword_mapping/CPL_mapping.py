...
class CPLMapping:
    ...
    def calculate_CBS(self):
        obj_cpl_bucketing = ProfitLossDataBucketing(df_datasheet=self.datasheet,df_nlp_bucket_master=self.df_nlp_bucket_master,notes_ref_dict=self.notes_ref_dict,notes_region_meta_data=self.notes_region_meta_data,standardised_cropped_dict=self.standardised_cropped_dict,standard_note_meta_dict=self.standard_note_meta_dict,transformed_standardised_cropped_dict= self.transformed_standardised_cropped_dict)
        obj_cpl_bucketing.fetch_report()
        self.pl_bucket_dict = obj_cpl_bucketing.bs_bucketing_dict
